...
```{r}
contraceptive_use <- read_csv("contraceptive_use.csv")
est <- read_csv("est.csv")
dat <- contraceptive_use %>%
  filter(is_in_union == "Y") %>%  
  mutate(
    year = round((start_date + end_date) / 2),  
    cp = contraceptive_use_modern * 100 ) %>%
  select(iso = division_numeric_code, year, cp) 
...
```{r}
# Load your package with plot_cp function
devtools::load_all()
...
devtools::document()
# Install and load Rd2md
library(Rd2md)

# Display the help file for plot_cp
as_markdown("man/plot_cp.Rd", outfile = "plot_cp.md")
